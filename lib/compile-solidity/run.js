...
async function run(rawSources, truffleConfig) {
  ...
  const compilerInput = prepareCompilerInput({
    sources,
    targets,
    settings: truffleConfig.compilers.solc.settings,
  });
  // perform compilation
  ...
  // handle warnings as errors if truffleConfig.strict
  // log if not truffleConfig.quiet
  ...
  if (errors.length > 0) {
    ...
    const formattedErrors = `${errors.trim()}\n\n\x1b[31mCompilation failed. See above.\x1b[0m`;
 ...
 */
async function invokeCompiler({ compilerInput, truffleConfig }) {
...
function detectErrors({
  ...
  const rawErrors = truffleConfig.strict
    ? outputErrors
    : outputErrors.filter(({ severity }) => severity !== "warning");

  const rawWarnings = truffleConfig.strict
  ...
  if (errors.includes("requires different compiler version")) {
    ...
    const configSolcVer =
      truffleConfig.compilers.solc.version || semverValid(solcVersion);
