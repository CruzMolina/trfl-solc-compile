...
class CompilerSupplier {
  constructor({ solcConfig, parser: _parser, quiet }) {
    ...
    if (quiet) this.strategyOptions.quiet = quiet;
  }
  ...
  load() {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      const isValidVersionRange = semverValidRange(userSpecification);
  ...
  async loadParserSolc(parser, solc) {
    if (parser) {
      ...
      const normalizedSolcVersion = semverCoerce(solcVersion).version;
  ...
  fileExists(localPath) {
    return existsSync(localPath) || isAbsolute(localPath);
  }
