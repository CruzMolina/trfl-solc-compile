...
function translateJsonCompilerOutput(output, libraries) {
  ...
  for (var contract in output["contracts"]) {
    ...
    if (gasEstimates["internal"]) {
      translatedGasEstimates["internal"] = translateGasEstimates(
        gasEstimates["internal"]
      );
    }
    if (gasEstimates["external"]) {
      translatedGasEstimates["external"] = translateGasEstimates(
        gasEstimates["external"]
      );
    }
    ...
    var contractOutput = {
      ...
      evm: {
        ...
        bytecode: {
          object:
            contractInput["bytecode"] &&
            linker.linkBytecode(contractInput["bytecode"], libraries || {}),
          opcodes: contractInput["opcodes"],
          sourceMap: contractInput["srcmap"],
          linkReferences:
            contractInput["bytecode"] &&
            linker.findLinkReferences(contractInput["bytecode"]),
        },
        deployedBytecode: {
          object:
            contractInput["runtimeBytecode"] &&
            linker.linkBytecode(
              contractInput["runtimeBytecode"],
              libraries || {}
            ),
          ...
          linkReferences:
            contractInput["runtimeBytecode"] &&
            linker.findLinkReferences(contractInput["runtimeBytecode"]),
        },
