...
class VersionRange extends LoadingStrategy {
  ...
  findNewestValidVersion(version, allVersions) {
    if (!semverValidRange(version)) return null;
    const satisfyingVersions = Object.keys(allVersions.releases)
      .map((solcVersion) => {
        if (semverSatisfies(solcVersion, version)) return solcVersion;
      })
      .filter((solcVersion) => solcVersion);
    if (satisfyingVersions.length > 0) {
      return satisfyingVersions.reduce((newestVersion, version) => {
        return semverGreater(version, newestVersion) ? version : newestVersion;
      }, "0.0.0");
    } else {
      return null;
    }
  ...
  getCachedSolcByVersionRange(version) {
    const cachedCompilerFileNames = readdirSync(this.compilerCachePath);
    const validVersions = cachedCompilerFileNames.filter((fileName) => {
      const match = fileName.match(/v\d+\.\d+\.\d+.*/);
      if (match) return semverSatisfies(match[0], version);
    });
  ...
  getMostRecentVersionOfCompiler(versions) {
    return versions.reduce((mostRecentVersionFileName, fileName) => {
      ...
      return semverGreater(match[0], mostRecentVersionMatch[0])
        ? fileName
  ...
  async getSolcByUrlAndCache(fileName, index = 0) {
    ...
    let downloadSolcSpinner;
    if (!this.config.quiet) {
      downloadSolcSpinner = ora({
        text: `Downloading compiler. Attempt #${index + 1}.`,
        color: "red",
      }).start();
    }
    try {
      const response = await (await fetch(url, { compress: true, timeout: 30000 })).text();
      if (!this.config.quiet) downloadSolcSpinner.succeed();
      this.addFileToCache(response, fileName);
      return this.compilerFromString(response);
    } catch (error) {
      if (!this.config.quiet) downloadSolcSpinner.fail();
  ...
  async getSolcFromCacheOrUrl(versionConstraint) {
    ...
    const isVersionRange = !semverValid(versionConstraint);
  ...
  getSolcVersions(index = 0) {
    ...
    let fetchSolcSpinner;
    if (!this.config.quiet) {
      fetchSolcSpinner = ora({
    ...
    if (!this.config.compilerRoots || this.config.compilerRoots.length < 1) {
      if (fetchSolcSpinner.isSpinning) fetchSolcSpinner.fail();
    ...
    const url =
      compilerRoots[compilerRoots.length] === "/"
        ...
        : `${compilerRoots[index]}/list.json`;
    return fetch(url, { compress: true, timeout: 30000 })
      .then((list) => {
        if (!this.config.quiet)
          if (fetchSolcSpinner.isSpinning) fetchSolcSpinner.succeed();
        return list.json();
      })
      .catch((error) => {
        if (!this.config.quiet)
  ...
  async load(versionRange) {
    const rangeIsSingleVersion = semverValid(versionRange);
  ...
  versionIsCached(version) {
    ...
    return cachedVersions.find((cachedVersion) =>
      semverSatisfies(cachedVersion, version)
    );
  }
}
