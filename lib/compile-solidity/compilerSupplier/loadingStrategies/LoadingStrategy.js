...
class LoadingStrategy {
  constructor(options) {
    ...
    const compilersDir = resolve(Config.getTruffleDataDirectory(), "compilers");
    const compilerCachePath = resolve(compilersDir, "node_modules"); // because babel binds to require & does weird things
    if (!existsSync(compilersDir)) mkdirSync(compilersDir);
    if (!existsSync(compilerCachePath)) mkdirSync(compilerCachePath); // for 5.0.8 users
  ...
  resolveCache(fileName) {
    return resolve(this.compilerCachePath, fileName);
  }
}
