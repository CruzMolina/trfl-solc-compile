...
const readAndParseArtifactFiles = (sourceFiles, contractsBuildDirectory) => {
  ...
  let buildFiles;
  try {
    buildFiles = readdirSync(contractsBuildDirectory);
  } catch (error) {
  ...
  buildFiles = buildFiles.filter((file) => extname(file) === ".json");
  const jsonData = buildFiles.map((file) => {
    const body = readFileSync(join(contractsBuildDirectory, file), "utf8");
    return { file, body };
  });
